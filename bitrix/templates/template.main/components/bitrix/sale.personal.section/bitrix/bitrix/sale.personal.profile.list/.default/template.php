...
<? if ($USER->IsAuthorized()): ?>
	<? if (count($arResult["PROFILES"])): ?>
		<table class="b-table sale-personal-profile-list-container">
			<tr>
				<? $dataColumns = array("ID", "DATE_UPDATE", "NAME", "PERSON_TYPE_ID"); ?>
			...
			<? foreach($arResult["PROFILES"] as $val): ?>
				<tr>
					...
					<td><?= $val["NAME"] ?></td>
					<td class="sale-personal-profile-list-types"><?= $val["PERSON_TYPE"]["NAME"] ?></td>
					<td class="sale-personal-profile-list-actions">
						<a class="g-button" title="<?= Loc::getMessage("SALE_DETAIL_DESCR") ?>"
