...
if(strlen($arResult["ID"])>0)
{
	...
	<form method="post"  class="col-md-12 sale-profile-detail-form" action="<?=POST_FORM_ACTION_URI?>" enctype="multipart/form-data">
		...
		<div class="col-md-offset-3 col-sm-9 sale-personal-profile-btn-block">
			<button type="submit" class="btn btn-themes btn-default btn-md g-button" name="save"><?echo GetMessage("SALE_SAVE") ?></button>
			&nbsp;
			<button type="submit" class="btn btn-themes btn-default btn-md g-decorated-link"  name="apply"><span><?=GetMessage("SALE_APPLY")?></span></button>
			&nbsp;
			<button type="submit" class="btn btn-themes btn-default btn-md g-decorated-link"  name="reset"><span><?echo GetMessage("SALE_RESET")?></span></button>
		</div>
	</form>
