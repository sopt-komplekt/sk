...
if ($normalCount > 0):
?>
<div class="basket_items_list" id="basket_items_list">
				...
				<tr>
					...
					foreach ($arResult["GRID"]["HEADERS"] as $id => $arHeader):
						...
						?>
							...
							<td class="item" id="col_<?=mb_strtolower($arHeader["id"]);?>">
						<?
						elseif ($arHeader["id"] == "PRICE"):
						?>
							<td class="price" id="col_<?=mb_strtolower($arHeader["id"]);?>">
						<?
						elseif ($arHeader["id"] == "QUANTITY"):
						?>
							<td class="quantity" id="col_<?=mb_strtolower($arHeader["id"]);?>">
						<?
						elseif (preg_match('/^PROPERTY_/', $arHeader['id'])):
						?>
							<td class="custom" id="col_<?=mb_strtolower($arHeader["id"]);?>">
						<?
						else:
						?>
							<td class="custom" id="col_<?=mb_strtolower($arHeader["id"]);?>">
					...
					>
						...
						foreach ($arResult["GRID"]["HEADERS"] as $id => $arHeader):
							...
							?>
								...
								<td class="item<? echo $arItem['AVAILABLE_QUANTITY'] <= 0 ? ' notavailable' : ''; ?>">
							...
							elseif (preg_match('/^PROPERTY_/', $arHeader['id'])):
							?>
								<td class="custom <?=mb_strtolower($arHeader["id"]);?>">
									<span><?=$arHeader["name"]; ?>:</span>
									<?
									if ($arHeader["id"] == "SUM"):
									?>
						...
						?>
							<td class="control">
								<? if ($bDeleteColumn): ?>
									<a class="delete" href="<?=str_replace("#ID#", $arItem["ID"], $arUrls["delete"])?>" onclick="return deleteProductRow(this)"><?=GetMessage("SALE_DELETE")?></a>
								<? endif; ?>
								<? if ($bDelayColumn): ?>
									<a class="delay" href="<?=str_replace("#ID#", $arItem["ID"], $arUrls["delay"])?>"><?=GetMessage("SALE_DELAY")?></a>
								<? endif; ?>
							</td>
						<?
						endif;
	...
	<div class="bx_ordercart_order_pay">
		...
		?>
			<div class="bx_ordercart_coupon">
				...
				<a class="g-button" href="javascript:void(0)" onclick="enterCoupon();" title="<?=GetMessage('SALE_COUPON_APPLY_TITLE'); ?>"><?=GetMessage('SALE_COUPON_APPLY'); ?></a>
			</div>
			<?
...
?>
		...
		<div class="bx_ordercart_order_pay_center">
			...
			<? if ($arParams["AUTO_CALCULATION"] != "Y"): ?>
				<a href="javascript:void(0)" onclick="updateBasket();"><?=GetMessage("SALE_REFRESH")?></a>
			<? endif; ?>
