...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		init: function(parameters)
		{
			...
			this.savedFilesBlockNode = BX('b-soa-saved-files');
			this.orderSaveBlockNode = BX('b-soa-orderSave');
			this.mainErrorsNode = BX('b-soa-main-notifications');
			...
			{
				this.totalInfoBlockNode = this.totalBlockNode.querySelector('.b-soa-cart-total');
				this.totalGhostBlockNode = this.totalBlockNode.querySelector('.b-soa-cart-total-ghost');
			}
		...
		sendRequest: function(action, actionData)
		{
			...
			{
				form = BX('b-soa-order-form');
				if (form)
					form.querySelector('input[type=hidden][name=sessid]').value = BX.bitrix_sessid();

				BX.ajax.submit(BX('b-soa-order-form'), BX.proxy(this.saveOrder, this));
			}
			else
				BX.ajax({
					method: 'POST',
		...
		showErrors: function(errors, scroll, showAll)
		{
			...
			{
				section = BX.findParent(errorNodes[k], {className: 'b-soa-section'});
		...
		checkNotifications: function()
		{
			...
			if (informer)
			{
				...
				{
					sections = this.orderBlockNode.querySelectorAll('.b-soa-section.bx-active');
		...
		{
			var sections = this.orderBlockNode.querySelectorAll('div.b-soa-section.bx-active'),
				i, errorNode;
		...
		showDeliveryItemsPage: function(page)
		{
			...
			deliveryItemsContainer = this.deliveryBlockNode.querySelector('.b-soa-pp-item-container');
		...
		showPaySystemItemsPage: function(page)
		{
			...
			paySystemItemsContainer = this.paySystemBlockNode.querySelector('.b-soa-pp-item-container');
		...
		{
			var firstSection = this.orderBlockNode.querySelector('.b-soa-section.bx-active');
		...
		locationsCompletion: function()
		{
			...
			for (i in this.locations)
			{
				...
				if (!this.firstLoad && this.options.propertyValidation)
				{
					if (inputStep)
					{
						...
						section = BX.findParent(locationNode, {className: 'b-soa-section'});
			...
			if (this.activeSectionId !== this.regionBlockNode.id)
				this.editFadeRegionContent(this.regionBlockNode.querySelector('.b-soa-section-content'));

			if (this.activeSectionId != this.propsBlockNode.id)
				this.editFadePropsContent(this.propsBlockNode.querySelector('.b-soa-section-content'));
		},
		...
		clickOrderSaveAction: function(event)
		{
			if (this.isValidForm())
			{
				...
				{
					BX.onCustomEvent('b-soa-order-save', []);
				}
				else
				{
					this.doSaveAction();
		...
		clickNextAction: function(event)
		{
			...
			{
				titleNode = section.next.querySelector('.b-soa-section-title-container');
				BX.bind(titleNode, 'click', BX.proxy(this.showByClick, this));
				editStep = section.next.querySelector('.b-soa-editstep');
				if (editStep)
					editStep.style.display = '';

				allSections = this.orderBlockNode.querySelectorAll('.b-soa-section.bx-active');
		...
		getNextSection: function(actionSection, skippedSection)
		{
			...
			var allSections = this.orderBlockNode.querySelectorAll('.b-soa-section.bx-active'),
				nextSection, i;
		...
		markSectionAsCompleted: function(section)
		{
			...
			)
			{
				...
				titleNode = section.querySelector('.b-soa-section-title-container');
				BX.bind(titleNode, 'click', BX.proxy(this.showByClick, this));
			}
		...
		{
			var sections = this.orderBlockNode.querySelectorAll('.b-soa-section.bx-active'),
				firstSection = sections[0],
				lastSection = sections[sections.length - 1],
				currentSection = BX.findParent(node, {className: "b-soa-section"}),
				isLastNode = false,
				buttons = [];
				...
				buttons.push(
					BX.create('A', {
						props: {
							...
							className: 'g-decorated-link pull-left'
						},
						html: '&larr; <span>'+this.params.MESS_BACK+'</span>',
						events: {
			...
			if (!isLastNode)
			{
				buttons.push(
					BX.create('A', {
						props: {
							href: 'javascript:void(0)',
							className: 'b-decorate-link pull-right g-button'
						},
						html: /*'<span>'+*/this.params.MESS_FURTHER/*+'</span>' &rarr;*/,
			...
			node.appendChild(
				BX.create('DIV', {
					props: {className: 'row b-soa-more'},
					children: [
						BX.create('DIV', {
							props: {className: 'col-xs-12 b-soa-more-btn'},
							children: buttons
						})
					]
				})
		...
		{
			// return BX.create('DIV', {props: {className: 'b-soa-section-content' + (!!notFluid ? '' : ' container-fluid')}});
			return BX.create('DIV', {props: {className: 'b-soa-section-content'}});
		},
		...
		switchOrderSaveButtons: function(state)
		{
			var orderSaveNode = this.orderSaveBlockNode,
				totalButton = this.totalBlockNode.querySelector('.b-soa-cart-total-button-container'),
				mobileButton = this.mobileTotalBlockNode.querySelector('.b-soa-cart-total-button-container'),
		...
		shouldBeSectionVisible: function(sections, currentPosition)
		{
			...
			for (; currentPosition < sections.length; currentPosition++)
			{
				...
				if (!this.firstLoad)
				{
					editStepNode = sections[currentPosition].querySelector('.b-soa-editstep');
		...
		{
			var sections = this.orderBlockNode.querySelectorAll('.b-soa-section[data-visited]'),
				i, state;
		...
		changeVisibleSection: function(section, state)
		{
			...
			{
				content = section.querySelector('.b-soa-section-content');
				if (content) {
					content.style.display = state ? '' : 'none';
				}
			}

			editStep = section.querySelector('.b-soa-editstep');
			if (editStep) {
		...
		editOrder: function()
		{
			...
			var sections = this.orderBlockNode.querySelectorAll('.b-soa-section.bx-active'), i;
			for (i in sections)
			{
				if (sections.hasOwnProperty(i))
				{
		...
		editSection: function(section)
		{
			...
			{
				BX.bind(titleNode, 'click', BX.proxy(this.showByClick, this));
				editButton = titleNode.querySelector('.b-soa-editstep');
		...
		editAuthBlock: function()
		{
			...
			var authContent = this.authBlockNode.querySelector('.b-soa-section-content'),
			...
			if (this.result.OK_MESSAGE && this.result.OK_MESSAGE.length)
			{
				...
				BX.prepend(okMessageNode, this.authBlockNode.querySelector('.b-soa-section-content'));
			}
		},
		...
		{
			if (!BX('b-soa-soc-auth-services'))
				return;

			var nodes = [],
				socServiceHiddenNode = BX('b-soa-soc-auth-services').querySelector('.bx-authform-social');
			...
			nodes.push(BX.create('DIV', {
				props: {className: 'b-soa-reg-block'},
				children: [
		...
		getAuthReference: function(authContent)
		{
			authContent.appendChild(
				BX.create('DIV', {
					...
					children: [
						BX.create('DIV', {
							props: {className: 'b-soa-reference col-xs-12'},
							children: [
		...
		toggleAuthForm: function(event)
		{
			...
			var target = event.target || event.srcElement,
				section = BX.findParent(target, {className: 'b-soa-section'}),
				container = BX.findParent(target, {className: 'b-soa-section-content'}),
			...
			section.appendChild(
				BX.create('DIV', {
					props: {className: 'b-soa-section-title-container'},
					children: [
						BX.create('h2', {
							props: {className: 'b-soa-section-title col-xs-7 col-sm-9'},
		...
		alignBasketColumns: function()
		{
			...
			{
				basketRows = this.basketBlockNode.querySelectorAll('.b-soa-basket-info');
				percent = 100;

				if (basketRows.length)
				{
					columnNodes = basketRows[0].querySelectorAll('.b-soa-item-properties');
					...
					if (columns > 0)
					{
						...
						{
							columnNodes = basketRows[i].querySelectorAll('.b-soa-item-properties')
			...
			{
				columnNodes = this.basketBlockNode.querySelectorAll('.b-soa-item-properties');
		...
		editActiveBasketBlock: function(activeNodeMode)
		{
			...
			{
				basketContent = node.querySelector('.b-soa-section-content');
				basketTable = BX.create('DIV', {props: {className: 'b-soa-item-table'}});
				...
				basketContent.appendChild(
					BX.create('DIV', {
						props: {className: 'b-soa-table-fade'},
						children: [
							BX.create('DIV', {
				...
				BX.bind(
					basketContent.querySelector('div.b-soa-table-fade').firstChild,
					'scroll',
		...
		editFadeBasketBlock: function(activeNodeMode)
		{
			...
			if (this.initialized.basket)
			{
				this.basketHiddenBlockNode.appendChild(node.querySelector('.b-soa-section-content'));
			...
			else
			{
				...
				BX.bind(
					this.basketBlockNode.querySelector('div.b-soa-table-fade').firstChild,
					'scroll',
					BX.proxy(this.basketBlockScrollCheckEvent, this)
				);
			}
		...
		editBasketItemsHeader: function(basketItemsNode)
		{
			...
			var headers = [
					BX.create('DIV', {
						props: {className: 'b-soa-item-td'},
						children: [
							BX.create('DIV', {
								props: {className: 'b-soa-item-td-title g-accent-font'},
			...
			for (i = 0; i < this.result.GRID.HEADERS.length; i++)
			{
				...
				headers.push(
					BX.create('DIV', {
						props: {className: 'b-soa-item-td b-soa-item-properties'},
						children: [
							BX.create('DIV', {
								props: {className: 'b-soa-item-td-title g-accent-font'},
								text: column.name + ':'
				...
				{
					headers.push(BX.create('DIV', {props: {className: 'b-soa-item-nth-4p1'}}));
					basketColumnIndex = 0;
				}
			}
			...
			basketItemsNode.appendChild(
				BX.create('DIV', {
					props: {className: 'b-soa-item-tr hidden-sm hidden-xs'},
					children: headers
				})
			);
		},
		...
		createBasketItem: function(basketItemsNode, item, index, active)
		{
			...
			for (i = 0; i < this.result.GRID.HEADERS.length; i++)
			{
				...
				if (basketColumnIndex == 4 && this.result.GRID.HEADERS[i + 1])
				{
					otherColumns.push(BX.create('DIV', {props: {className: 'b-soa-item-nth-4p1'}}));
					basketColumnIndex = 0;
				}
			}
			...
			cols = [
				BX.create('DIV', {
					...
					children: [
						BX.create('DIV', {
							props: {className: 'b-soa-item-block'},
			...
			basketItemsNode.appendChild(
				BX.create('DIV', {
					props: {className: 'b-soa-item-tr b-soa-basket-info' + (index == 0 ? ' b-soa-item-tr-first' : '')},
					children: cols
				})
			);
			...
			if (hiddenColumns.length)
			{
				basketItemsNode.appendChild(
					BX.create('DIV', {
						props: {className: 'b-soa-item-tr b-soa-item-info-container'},
						children: [
							BX.create('DIV', {
								props: {className: 'b-soa-item-td'},
								children: [
									BX.create('A', {
										props: {href: '', className: 'b-soa-info-shower'},
									...
									BX.create('DIV', {
										props: {className: 'b-soa-item-info-block'},
										children: [
											BX.create('TABLE', {
												props: {className: 'b-soa-info-block'},
		...
		showAdditionalProperties: function(event)
		{
			var target = event.target || event.srcElement,
				...
				parentContainer = BX.findParent(target, {className: 'b-soa-item-tr b-soa-item-info-container'}),
		...
		createBasketItemImg: function(data)
		{
			...
			logoNode = BX.create('DIV', {props: {className: 'b-soa-item-imgcontainer'}});
			...
			return BX.create('DIV', {
				props: {className: 'b-soa-item-img-block'},
				children: [logoNode]
			});
		},
		...
		createBasketItemContent: function(data)
		{
			...
			if (data.DETAIL_PAGE_URL && data.DETAIL_PAGE_URL.length)
				titleHtml = '<a class="g-decorated-link" href="' + data.DETAIL_PAGE_URL + '"><span>' + titleHtml + '</span></a>';
			...
			if (this.options.showPropsInBasket && props.length)
			{
				for (var i in props)
				{
					if (props.hasOwnProperty(i))
					{
						...
						propsNodes.push(
							BX.create('DIV', {
								props: {className: 'b-soa-item-td-title'},
						...
						propsNodes.push(
							BX.create('DIV', {
								props: {className: 'b-soa-item-td-text'},
								style: {textAlign: 'left'},
								text: value
							})
						);
			...
			return BX.create('DIV', {
				props: {className: 'b-soa-item-content'},
				children: propsNodes.length ? [
					BX.create('DIV', {props: {className: 'b-soa-item-title'}, html: titleHtml}),
		...
		createBasketItemColumn: function(column, allData, active)
		{
			...
			var data = allData.columns[column.id] ? allData.columns : allData.data,
				toRight = BX.util.in_array(column.id, ["QUANTITY", "PRICE_FORMATED", "DISCOUNT_PRICE_PERCENT_FORMATED", "SUM"]),
				textNode = BX.create('DIV', {props: {className: 'b-soa-item-td-text'}}),
				logotype, img;
			...
			return BX.create('DIV', {
				...
				children: [
					BX.create('DIV', {
						props: {className: 'b-soa-item-td-title visible-xs visible-sm'},
						text: column.name
					}),
					textNode
				]
		...
		createBasketItemHiddenColumn: function(column, allData)
		{
			...
			var data = allData.columns[column.id] ? allData.columns : allData.data,
				textNode = BX.create('TD', {props: {className: 'b-soa-info-text'}}),
				logotype, img, i;
				...
				if (props && props.length)
				{
					for (i in props)
					{
						if (props.hasOwnProperty(i))
						{
							...
							propsNodes.push(
								BX.create('TR', {
									props: {className: 'b-soa-info-line'},
									children: [
										BX.create('TD', {props: {className: 'b-soa-info-title'}, text: name + ':'}),
										BX.create('TD', {props: {className: 'b-soa-info-text'}, html: BX.util.htmlspecialchars(value)})
									]
								})
							);
						}
			...
			return BX.create('TR', {
				...
				children: [
					BX.create('TD', {
						props: {className: 'b-soa-info-title'},
						text: column.name + ':'
					}),
					textNode
				]
		...
		popupShow: function(e, url, source)
		{
			...
			var that = this;
			this.popup = new BX.PopupWindow('b-soa-image-popup', null, {
				lightShadow: true,
				offsetTop: 0,
				offsetLeft: 0,
				...
				events: {
					onPopupShow: function() {
						BX.create("IMG", {
							...
							events: {
								load: function() {
									var content = BX('b-soa-image-popup-content');
									if (content)
									{
				...
				content: BX.create('DIV', {
					props: {id: 'b-soa-image-popup-content'},
		...
		editCoupons: function(basketItemsNode)
		{
			var couponsList = this.getCouponsList(true),
				...
				couponsBlock = BX.create('DIV', {
					props: {className: 'b-soa-coupon-block'},
					children: [
						BX.create('DIV', {
							props: {className: 'b-soa-coupon-input'},
							children: [
								BX.create('INPUT', {
									props: {
										id: 'coupon',
						...
						BX.create('SPAN', {props: {className: 'b-soa-coupon-item'}, children: couponsList})
					]
				});

			basketItemsNode.appendChild(
				BX.create('DIV', {
					props: {className: 'b-soa-coupon'},
					children: [
						couponsLabel,
						couponsBlock
					]
		...
		editCouponsFade: function(basketItemsNode)
		{
			...
			if (couponsList.length)
			{
				...
				couponsBlock = BX.create('DIV', {
					...
					children: [
						BX.create('DIV', {
							props: {className: 'b-soa-coupon-list'},
							children: [
								BX.create('DIV', {
									props: {className: 'b-soa-coupon-item'},
				...
				basketItemsNode.appendChild(
					BX.create('DIV', {
						props: {className: 'b-soa-coupon b-soa-coupon-item-fixed'},
		...
		getCouponNode: function(coupon, active)
		{
			...
			return BX.create('STRONG', {
				attrs: {
					...
					className: 'b-soa-coupon-item-' + couponItem
				},
				children: active ? [
					couponName || '',
					BX.create('SPAN', {
						props: {className: 'b-soa-coupon-remove'},
						events: {
							click: BX.delegate(function(e){
								var target = e.target || e.srcElement,
					...
					BX.create('SPAN', {
						props: {
							className: 'b-soa-tooltip b-soa-tooltip-coupon b-soa-tooltip-' + tooltip + ' tooltip top'
						},
						children: [
		...
		{
			return BX.create('DIV', {
				props: {className: 'b-soa-coupon-label'},
				children: active
		...
		{
			var couponListNodes = this.orderBlockNode.querySelectorAll('.b-soa-coupon:not(.b-soa-coupon-item-fixed) .b-soa-coupon-item'),
			...
			for (i = 0; i < couponListNodes.length; i++)
			{
				...
				couponListNodes[i].appendChild(this.getCouponNode({text: coupon}, true, 'b-soa-coupon-item-danger'));
			}
		...
		editActiveRegionBlock: function(activeNodeMode)
		{
			...
			{
				regionContent = node.querySelector('.b-soa-section-content');
				if (!regionContent)
				{
		...
		{
			var regionContent = this.regionBlockNode.querySelector('.b-soa-section-content'), newContent;
		...
		getDeliveryLocationInput: function(node)
		{
			...
			if (location && location[0] && location[0].output)
			{
				...
				labelHtml = '<label class="b-soa-custom-label" for="soa-property-' + locationId + '">'
				...
				insertedLoc = BX.create('DIV', {
					...
					props: {className: 'form-group b-soa-location-input-container'},
			...
			if (altProperty)
			{
				...
				label = BX.create('LABEL', {
					...
					props: {className: 'b-soa-custom-label'},
					html: labelTextHtml
				});
				...
				input = BX.create('INPUT', {
					props: {
						...
						autocomplete: 'city',
						className: 'form-control b-soa-customer-input bx-ios-fix',
			...
			if (location && location[0])
			{
				node.appendChild(
					BX.create('DIV', {
						props: {className: 'b-soa-reference'},
		...
		getZipLocationInput: function(node)
		{
			...
			if (zipProperty)
			{
				...
				propsItemNode = BX.create('DIV', {props: {className: "form-group b-soa-location-input-container"}});
		...
		getPersonTypeControl: function(node)
		{
			...
			if (personTypesCount > 1)
			{
				input = BX.create('DIV', {
					...
					children: [
						BX.create('LABEL', {props: {className: 'b-soa-custom-label'}, html: this.params.MESS_PERSON_TYPE}),
						BX.create('BR')
					]
				});
		...
		getProfilesControl: function(node)
		{
			...
			if (profilesLength && this.params.ALLOW_USER_PROFILES === 'Y')
			{
				...
				{
					label = BX.create('LABEL', {props: {className: 'b-soa-custom-label'}, html: this.params.MESS_SELECT_PROFILE});
		...
		editActivePaySystemBlock: function(activeNodeMode)
		{
			...
			{
				paySystemContent = node.querySelector('.b-soa-section-content');
				if (!paySystemContent)
				{
				...
				paySystemNode = BX.create('DIV', {props: {className: 'row b-soa-pp'}});
		...
		{
			var paySystemContent = this.paySystemBlockNode.querySelector('.b-soa-section-content'), newContent;
		...
		editPaySystemItems: function(paySystemNode)
		{
			...
			var paySystemItemsContainer = BX.create('DIV', {props: {className: 'col-sm-12 b-soa-pp-item-container'}}),
				paySystemItemNode, i;
		...
		createPaySystemItem: function(item)
		{
			var checked = item.CHECKED == 'Y',
				...
				input, title, description, itemNode;

			logoNode = BX.create('DIV', {props: {className: 'b-soa-pp-company-image'}});
			...
			input = BX.create('INPUT', {
				props: {
					...
					name: 'PAY_SYSTEM_ID',
					type: 'checkbox',
					className: 'b-soa-pp-company-checkbox',
					value: paySystemId,
					checked: checked
				}
			});
			...
			if (this.params.SHOW_PAY_SYSTEM_LIST_NAMES == 'Y')
			{
				title = BX.create('H3', {
					props: {
						className: 'b-soa-pp-company-smalltitle g-accent-font'
					}, 
					text: item.NAME
				});
			}
			...
			description = BX.create('DIV', {
				props: {className: 'b-soa-pp-company-description'},
				html: item.DESCRIPTION
			});

			itemNode = BX.create('DIV', {
				props: {className: 'b-soa-pp-company'},
				children: [input, title, description],
				events: {
		...
		// 			props: {
		// 				className: (this.result.PAY_SYSTEM.length == 0 ? 'col-sm-12' : 'col-sm-5') + ' b-soa-pp-desc-container'
		// 			}
		// 		}),
		...
		// 				props: {className: 'b-soa-pp-company-subTitle'},
		// 				text: currentPaySystem.NAME
		// 			});
		// 		}

		// 		label = BX.create('DIV', {
		// 			props: {className: 'b-soa-pp-company-logo'},
		// 			children: [
		// 				BX.create('DIV', {
		// 					props: {className: 'b-soa-pp-company-graf-container'},
		// 					children: [logoNode]
		// 				})
		// 			]
		// 		});
		...
		// 		title = BX.create('DIV', {
		// 			props: {className: 'b-soa-pp-company-block'},
		// 			children: [BX.create('DIV', {props: {className: 'b-soa-pp-company-desc'}, html: currentPaySystem.DESCRIPTION})]
		// 		});

		// 		if (currentPaySystem.PRICE && parseFloat(currentPaySystem.PRICE) > 0)
		// 		{
		// 			price = BX.create('UL', {
		// 				props: {className: 'b-soa-pp-list'},
		// 				children: [
		// 					BX.create('LI', {
		// 						children: [
		// 							BX.create('DIV', {props: {className: 'b-soa-pp-list-termin'}, html: this.params.MESS_PRICE + ':'}),
		// 							BX.create('DIV', {props: {className: 'b-soa-pp-list-description'}, text: '~' + currentPaySystem.PRICE_FORMATTED})
		// 						]
		// 					})
		// 				]
		// 			});
		...
		getInnerPaySystem: function()
		{
			...
			if (paySystem.DESCRIPTION && paySystem.DESCRIPTION.length)
			{
				title = BX.create('DIV', {
					props: {className: 'b-soa-pp-company-block'},
					children: [BX.create('DIV', {props: {className: 'b-soa-pp-company-desc'}, html: paySystem.DESCRIPTION})]
				});
			}
			...
			innerPsDesc = BX.create('DIV', {props: {className: 'b-soa-pp-company-desc'}, html: htmlString});

			return BX.create('DIV', {
				props: {className: 'b-soa-pp-inner-ps' + (isSelected ? ' bx-selected' : '')},
		...
		editFadePaySystemContent: function(node)
		{
			...
			if (this.isSelectedInnerPayment())
			{
				...
				addedHtml += '<div class="b-soa-pp-company-selected">';
		...
		selectPaySystem: function(event)
		{
			...
			var target = event.target || event.srcElement,
				innerPaySystemSection = this.paySystemBlockNode.querySelector('div.b-soa-pp-inner-ps'),
			...
			var innerPsAction = BX.hasClass(target, 'b-soa-pp-inner-ps') ? target : BX.findParent(target, {className: 'b-soa-pp-inner-ps'}),
				actionSection = BX.hasClass(target, 'b-soa-pp-company') ? target : BX.findParent(target, {className: 'b-soa-pp-company'}),
			...
			else if (actionSection)
			{
				...
				{
					selectedSection = this.paySystemBlockNode.querySelector('.b-soa-pp-company.bx-selected');
					BX.addClass(actionSection, 'bx-selected');
					actionInput = actionSection.querySelector('input[type=checkbox]');
					actionInput.checked = true;
		...
		editActiveDeliveryBlock: function(activeNodeMode)
		{
			...
			{
				deliveryContent = node.querySelector('.b-soa-section-content');
				if (!deliveryContent)
				{
				...
				deliveryNode = BX.create('DIV', {props: {className: 'row b-soa-pp'}});
		...
		editDeliveryItems: function(deliveryNode)
		{
			...
			var deliveryItemsContainer = BX.create('DIV', {props: {className: 'col-sm-12 b-soa-pp-item-container'}}),
				deliveryItemNode, k;
		...
		// 	var deliveryInfoContainer = BX.create('DIV', {props: {className: 'col-sm-5 b-soa-pp-desc-container'}}),
		...
		// 		subTitle = BX.create('DIV', {props: {className: 'b-soa-pp-company-subTitle'}, text: name});
		...
		// 		props: {className: 'b-soa-pp-company-block'},
		// 		children: [
		// 			BX.create('DIV', {props: {className: 'b-soa-pp-company-desc'}, html: currentDelivery.DESCRIPTION}),
		// 			currentDelivery.CALCULATE_DESCRIPTION
		// 				? BX.create('DIV', {props: {className: 'b-soa-pp-company-desc'}, html: currentDelivery.CALCULATE_DESCRIPTION})
		// 				: null
		// 		]
		// 	});
		...
		// 			children: [
		// 				BX.create('DIV', {
		// 					props: {className: 'b-soa-pp-list-termin'},
		// 					html: this.params.MESS_PRICE + ':'
		// 				}),
		// 				BX.create('DIV', {
		// 					props: {className: 'b-soa-pp-list-description'},
		...
		// 		period = BX.create('LI', {
		// 			children: [
		// 				BX.create('DIV', {props: {className: 'b-soa-pp-list-termin'}, html: this.params.MESS_PERIOD + ':'}),
		// 				BX.create('DIV', {props: {className: 'b-soa-pp-list-description'}, html: currentDelivery.PERIOD_TEXT})
		// 			]
		// 		});
		// 	}

		// 	clear = BX.create('DIV', {style: {clear: 'both'}});
		// 	infoList = BX.create('UL', {props: {className: 'b-soa-pp-list'}, children: [price, period]});
			...
			for (i in delivery.EXTRA_SERVICES)
			{
				...
				if (currentService.editControl.indexOf('this.checked') == -1)
				{
					...
					serviceNode = BX.create('DIV', {
						props: {className: 'form-group b-soa-pp-field'},
						html: currentService.editControl
						+ (currentService.description && currentService.description.length
							? '<div class="b-soa-service-small">' + BX.util.htmlspecialchars(currentService.description) + '</div>'
							: '')
					});
		...
		{
			var deliveryContent = this.deliveryBlockNode.querySelector('.b-soa-section-content'), newContent;
		...
		createDeliveryItem: function(item)
		{
			var checked = item.CHECKED == 'Y',
				...
				input, title, description, cost, itemNode;
			...
			{
				title = BX.create('H3', {
					props: {className: 'b-soa-pp-company-smalltitle g-accent-font'},
			...
			{
				cost = BX.create('B', {
					props: {className: 'b-soa-pp-delivery-cost g-accent-color'},
			...
			itemNode = BX.create('DIV', {
				props: {className: 'b-soa-pp-company'},
				children: [input, title, cost, description],
		...
		editFadeDeliveryContent: function(node)
		{
			...
			if (selectedDelivery && selectedDelivery.NAME)
			{
				...
				node.appendChild(
					BX.create('DIV', {
						props: {className: 'col-sm-9 b-soa-pp-company-selected'},
						children: arNodes
					})
				);
				node.appendChild(
					BX.create('DIV', {
						props: {className: 'col-sm-3 b-soa-pp-price'},
		...
		selectDelivery: function(event)
		{
			...
			var target = event.target || event.srcElement,
				actionSection =  BX.hasClass(target, 'b-soa-pp-company') ? target : BX.findParent(target, {className: 'b-soa-pp-company'}),
				selectedSection = this.deliveryBlockNode.querySelector('.b-soa-pp-company.bx-selected'),
		...
		activatePickUp: function(deliveryName)
		{
			...
			this.pickUpBlockNode.querySelector('h2.b-soa-section-title').innerHTML =
				'<span class="b-soa-section-title-count"></span>' + BX.util.htmlspecialchars(deliveryName);
		...
		editActivePickUpBlock: function(activeNodeMode)
		{
			...
			{
				pickUpContent = node.querySelector('.b-soa-section-content');
				if (!pickUpContent)
				{
		...
		{
			var pickUpContent = this.pickUpBlockNode.querySelector('.b-soa-section-content'), newContent;
		...
		editFadePickUpContent: function(pickUpContainer)
		{
			...
			if (selectedPickUp)
			{
				if (this.params.SHOW_STORES_IMAGES == 'Y')
				{
					...
					html += '<img src="' + imgSrc + '" class="b-soa-pickup-preview-img">';
				}
				...
				if (this.params.SHOW_STORES_IMAGES == 'Y')
				{
					BX.bind(pickUpContainer.querySelector('.b-soa-pickup-preview-img'), 'click', BX.delegate(function(e){
		...
		editPickUpList: function(isNew)
		{
			...
			var pickUpList = BX.create('DIV', {props: {className: 'b-soa-pickup-list main'}}),
			...
			recommendList = this.pickUpBlockNode.querySelector('.b-soa-pickup-list.recommend');
			if (!recommendList)
				recommendList = this.pickUpHiddenBlockNode.querySelector('.b-soa-pickup-list.recommend');

			if (!recommendList || !recommendList.querySelector('.b-soa-pickup-list-item.bx-selected'))
			{
			...
			if (!!isNew)
			{
				...
				container.appendChild(
					BX.create('DIV', {
						props: {className: 'b-soa-pickup-subTitle'},
			...
			{
				container = this.pickUpBlockNode.querySelector('.b-soa-pickup-list.main');
		...
		createPickUpItem: function(currentStore, options)
		{
			...
			var imgClassName = 'b-soa-pickup-l-item-detail',
				buttonClassName = 'b-soa-pickup-l-item-btn',
			...
			if (this.params.SHOW_STORES_IMAGES === 'Y')
			{
				...
				logoNode = BX.create('IMG', {
					props: {
						src: imgSrc,
						className: 'b-soa-pickup-l-item-img'
					},
					events: {
						click: BX.delegate(function(e){
							this.popupShow(e, logotype && logotype.src_orig || imgSrc);
			...
			storeNode = BX.create('DIV', {
				props: {className: 'b-soa-pickup-list-item', id: 'store-' + currentStore.ID},
				children: [
					BX.create('DIV', {
						props: {className: 'b-soa-pickup-l-item-adress'},
					...
					BX.create('DIV', {
						...
						children: [
							logoNode,
							BX.create('DIV', {props: {className: 'b-soa-pickup-l-item-name'}, text: currentStore.TITLE}),
							BX.create('DIV', {props: {className: 'b-soa-pickup-l-item-desc'}, html: html})
						]
					}),
					BX.create('DIV', {
		...
		editPickUpRecommendList: function(geoLocation)
		{
			...
			var recommendList = BX.create('DIV', {props: {className: 'b-soa-pickup-list recommend'}}),
				buyerStoreInput = BX('BUYER_STORE'),
				selectedDelivery = this.getSelectedDelivery();
		...
		selectStore: function(event)
		{
			...
			else
			{
				...
				storeItem = BX.hasClass(target, 'b-soa-pickup-list-item')
					? target
					: BX.findParent(target, {className: 'b-soa-pickup-list-item'});
			}
		...
		editActivePropsBlock: function(activeNodeMode)
		{
			...
			{
				propsContent = node.querySelector('.b-soa-section-content');
				if (!propsContent)
				{
		...
		{
			var propsContent = this.propsBlockNode.querySelector('.b-soa-section-content'), newContent;
		...
		editPropsItems: function(propsNode)
		{
			...
			var propsItemsContainer = BX.create('DIV', {props: {className: 'col-sm-12 b-soa-customer'}}),
				group, property, groupIterator = this.propertyCollection.getGroupIterator(), propsIterator;

			if (!propsItemsContainer)
				propsItemsContainer = this.propsBlockNode.querySelector('.col-sm-12.b-soa-customer');
		...
		getPropertyRowNode: function(property, propsItemsContainer, disabled)
		{
			...
			{
				BX.addClass(propsItemNode, "form-group b-soa-customer-field");
		...
		insertLocationProperty: function(property, propsItemNode, disabled)
		{
			...
			if (property.getId() in this.locations)
			{
				if (disabled)
				{
					...
					if (propRow)
					{
						propNodes = propRow.querySelectorAll('div.b-soa-loc');
				...
				else
				{
					...
					for (i = 0; i < propRow.length; i ++)
					{
						...
						insertedLoc = BX.create('DIV', {props: {className: 'b-soa-loc'}, html: currentLocation.HTML});
		...
		addLocationProperty: function(e)
		{
			...
			if (BX.hasClass(lastProp, 'b-soa-loc'))
			{
		...
		alterProperty: function(settings, propContainer)
		{
			...
			for (i = 0; i < inputs.length; i++)
			{
				...
				BX.addClass(inputs[i], 'form-control b-soa-customer-input bx-ios-fix');
			}
			...
			if (add.length)
			{
				...
				BX.bind(add, 'click', BX.delegate(function(e){
					...
					if (textInputs.length)
					{
						...
						BX.addClass(textInputs[textInputs.length - 1], 'form-control b-soa-customer-input bx-ios-fix');
			...
			if (propsErrors.length && !navigated)
			{
				if (this.activeSectionId == this.propsBlockNode.id)
				{
					...
					for (i = 0; i < tooltips.length; i++)
					{
						...
						{
							this.animateScrollTo(BX.findParent(tooltips[i], {className: 'form-group b-soa-customer-field'}), 800, 50);
							break;
						}
					}
				}
		...
		isValidRegionBlock: function()
		{
			...
			var regionProps = this.orderBlockNode.querySelectorAll('.b-soa-location-input-container[data-property-id-row]'),
				regionErrors = [],
		...
		isValidPropertiesBlock: function(excludeLocation)
		{
			...
			var props = this.orderBlockNode.querySelectorAll('.b-soa-customer-field[data-property-id-row]'),
				propsErrors = [],
			...
			else
			{
				...
				tooltip = BX.create('DIV', {
					props: {
						...
						className: 'b-soa-tooltip b-soa-tooltip-static b-soa-tooltip-danger tooltip top'
					},
					children: [
		...
		editPropsComment: function(propsNode)
		{
			...
			label = BX.create('LABEL', {
				...
				props: {className: 'b-soa-customer-label'},
			...
			input = BX.create('TEXTAREA', {
				props: {
					id: 'orderDescription',
					cols: '4',
					className: 'form-control b-soa-customer-textarea bx-ios-fix',
					name: 'ORDER_DESCRIPTION'
				},
				text: this.result.ORDER_DESCRIPTION ? this.result.ORDER_DESCRIPTION : ''
			});
			div = BX.create('DIV', {
				props: {className: 'form-group b-soa-customer-field'},
		...
		editTotalBlock: function()
		{
			...
			if (!this.result.SHOW_AUTH)
			{
				this.totalInfoBlockNode.appendChild(
					BX.create('DIV', {
						props: {className: 'b-soa-cart-total-button-container' + (!showOrderButton ? ' visible-xs' : '')},
						children: [
							BX.create('A', {
								props: {
									href: 'javascript:void(0)',
									className: 'g-button g-button--accent'
								},
							...
							BX.create('DIV', {
								props: {
									className: 'b-soa-order-agreement'
								},
								html: 'Нажимая на кнопку «' + this.params.MESS_ORDER +'», я даю своё <!--noindex--><a rel="nofollow" href="/agreement/" class="g-ajax-data" target="_blank">согласие</a><!--/noindex--> на обработку персональных данных.'
							})
						]
					})
				);
		...
		editMobileTotalBlock: function()
		{
			...
			BX.bind(this.mobileTotalBlockNode.querySelector('a.b-soa-price-not-calc'), 'click', BX.delegate(function(){
		...
		{
			var totalValue, className = 'b-soa-cart-total-line';
			...
			if (params.error)
			{
				totalValue = [BX.create('A', {
					props: {className: 'b-soa-price-not-calc'},
					html: value,
					events: {
			...
			else if (params.free)
			{
				totalValue = [BX.create('SPAN', {
					props: {className: 'b-soa-price-free'},
					html: value
				})];
			}
			else
			...
			if (params.total)
			{
				className += ' b-soa-cart-total-line-total';
			}

			if (params.highlighted)
			{
				className += ' b-soa-cart-total-line-highlighted';
			}

			return BX.create('DIV', {
				props: {className: className},
				children: [
					BX.create('SPAN', {props: {className: 'b-soa-cart-t'}, text: name}),
					BX.create('SPAN', {
						props: {
							className: 'b-soa-cart-d' + (!!params.total && this.options.totalPriceChanged ? ' b-soa-changeCostSign' : '')
						},
						children: totalValue
					})
				]
		...
		basketBlockScrollCheckEvent: function(e)
		{
			...
			if (scrollLeft == 0)
				BX.removeClass(parent, 'b-soa-table-fade-left');
			else
				BX.addClass(parent, 'b-soa-table-fade-left');

			if (scrollRight == 0)
				BX.removeClass(parent, 'b-soa-table-fade-right');
			else
				BX.addClass(parent, 'b-soa-table-fade-right');
		},

		basketBlockScrollCheck: function()
		{
			var scrollableNodes = this.orderBlockNode.querySelectorAll('div.b-soa-table-fade'),
			...
			for (i = 0; i < scrollableNodes.length; i++)
			{
				...
				tableNode = parentNode.querySelector('div.b-soa-item-table');
				...
				if (scrollable)
				{
					...
					if (scrollLeft == 0)
						BX.removeClass(parentNode, 'b-soa-table-fade-left');
					else
						BX.addClass(parentNode, 'b-soa-table-fade-left');

					if (scrollRight == 0)
						BX.removeClass(parentNode, 'b-soa-table-fade-right');
					else
						BX.addClass(parentNode, 'b-soa-table-fade-right');

					if (scrollLeft == 0 && scrollRight == 0)
						BX.addClass(parentNode, 'b-soa-table-fade-right');
				}
				else
					BX.removeClass(parentNode, 'b-soa-table-fade-left b-soa-table-fade-right');
			}
		},
		...
		totalBlockScrollCheck: function()
		{
			...
			if (ghostBottom - this.totalBlockNode.offsetHeight < scrollTop + 20)
				BX.addClass(this.totalInfoBlockNode, 'b-soa-cart-total-bottom');
			else
				BX.removeClass(this.totalInfoBlockNode, 'b-soa-cart-total-bottom');

			if (scrollTop > ghostTop && !BX.hasClass(this.totalInfoBlockNode, 'b-soa-cart-total-fixed'))
			{
				width = this.totalInfoBlockNode.offsetWidth;
				BX.addClass(this.totalInfoBlockNode, 'b-soa-cart-total-fixed');
			...
			else if (scrollTop < ghostTop && BX.hasClass(this.totalInfoBlockNode, 'b-soa-cart-total-fixed'))
			{
				BX.removeClass(this.totalInfoBlockNode, 'b-soa-cart-total-fixed');
		...
		totalBlockResizeCheck: function()
		{
			...
			if (BX.hasClass(this.totalInfoBlockNode, 'b-soa-cart-total-fixed')) {
		...
		{
			var totalNode = this.totalInfoBlockNode.querySelector('.b-soa-cart-total-line.b-soa-cart-total-line-total'),
			...
			{
				buttonNode = this.totalInfoBlockNode.querySelector('.b-soa-cart-total-button-container');
				if (buttonNode)
				{
