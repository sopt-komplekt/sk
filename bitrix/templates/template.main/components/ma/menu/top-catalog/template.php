		...
		<? if($arItem['DEPTH_LEVEL'] <= $arParams['MAX_LEVEL']): ?>
			...
			<? if($arItem["IS_PARENT"]): ?>
				...
				<? if($arItem["DEPTH_LEVEL"] == 1): ?>
					<li class="li-deep-<?=$arItem["DEPTH_LEVEL"]?> <?=$li_clname?><?if($arItem["SELECTED"]):?> root-active<?endif?>">
						<a href="<?=$arItem["LINK"]?>" class="a-deep-<?=$arItem["DEPTH_LEVEL"]?><?if($arItem["SELECTED"]):?> root-active<?endif?> <?=$a_clname?>">
							<?=$arItem["TEXT"]?>
						</a>
				...
				<? else: ?>
					<li class="li-deep-<?=$arItem["DEPTH_LEVEL"]?> <?=$li_clname?><?if($arItem["SELECTED"]):?> active<?endif?>">
						<a href="<?=$arItem["LINK"]?>" class="a-deep-<?=$arItem["DEPTH_LEVEL"]?><?if($arItem["SELECTED"]):?> active<?endif?> <?=$a_clname?>">
			...
			<? else: ?>
				...
				<? else: ?>
					...
					<? if($arItem["DEPTH_LEVEL"] == 1): ?>
						<li class="li-deep-<?=$arItem["DEPTH_LEVEL"]?> <?=$li_clname?><?if($arItem["SELECTED"]):?> root-active<?endif?>">
							<a href="" class="a-deep-<?=$arItem["DEPTH_LEVEL"]?><?if($arItem["SELECTED"]):?> root-active<?endif?> <?=$a_clname?>" title="<?=GetMessage("MENU_ITEM_ACCESS_DENIED")?>">
								<?=$arItem["TEXT"]?>
							</a>
						</li>
					<? else: ?>
						<li class="li-deep-<?=$arItem["DEPTH_LEVEL"]?> <?=$li_clname?><?if($arItem["SELECTED"]):?> active<?endif?>">
							<a href="" class="a-deep-<?=$arItem["DEPTH_LEVEL"]?> denied<?if($arItem["SELECTED"]):?> active<?endif?> <?=$a_clname?>" title="<?=GetMessage("MENU_ITEM_ACCESS_DENIED")?>">
								<span><?=$arItem["TEXT"]?></span>
							</a>
						</li>
					<? endif; ?>
