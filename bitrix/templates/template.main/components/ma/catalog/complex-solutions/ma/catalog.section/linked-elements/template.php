...
<div class="b-products-table<?if (count($arResult['ITEMS']) > 3):?> b-products-table--has-bg<? endif;?>">
	<div class="b-products-table__caption">
		<div class="b-products-table__picture-title"></div>
		<div class="b-products-table__info-title">Наименование</div>
		<div class="b-products-table__quantity-title">Количество, шт</div>
		<div class="b-products-table__price-title">Цена, <span class="ruble">p</span></div>
		<div class="b-products-table_summ-price-title">Сумма, <span class="ruble">p</span></div>
	</div>
	<? 
	...
	?><??>
		<div class="b-products-table__item b-sebi-<?=$arItem['ID']?> jsProductTable" id="<?=$strMainID;?>">
			<div class="b-products-table__picture">
				<a class="b-products-table__picture-img"  href="<?=$arItem['DETAIL_PAGE_URL']; ?>" title="<?=$imgTitle;?>" style="background-image: url('<? echo $arItem['PREVIEW_PICTURE']['SRC'] ? $arItem['PREVIEW_PICTURE']['SRC'] : $templateFolder.'/images/no_photo@1x.png'?>')" id="<? echo $arItemIDs['PICT']; ?>"></a>
				<? if ($arParams['SHOW_DISCOUNT_PERCENT'] == 'Y' && $minPrice['DISCOUNT_DIFF_PERCENT'] > 0): ?>
					<div class="b-products-table__discount" id="<?=$arItemIDs['DSC_PERC'];?>">-<?=$minPrice['DISCOUNT_DIFF_PERCENT'];?>%</div>
				<? endif; ?>
				<? if ($arItem['LABEL']): ?>
					<div class="b-products-table__label" title="<?=$arItem['LABEL_VALUE']; ?>" id="<?=$arItemIDs['STICKER_ID'];?>">Хит</div>
				<? endif; ?>
			</div>

			<div class="b-products-table__info">

				<div class="b-products-table__title">
				...
				<? else: ?>
					...
					?>
								...
								<div class="b-products-table__scu" id="<? echo $arItemIDs['PROP_DIV']; ?>">
									<?
								...
								if ($arItem['OFFERS_PROPS_DISPLAY'])
								{
									foreach ($arItem['JS_OFFERS'] as $keyOffer => $arJSOffer)
									{
										...
										if (!empty($arJSOffer['DISPLAY_PROPERTIES']))
										{
											...
											{
												$strProps .= '<div class="b-products-table__articul-item"><b>'.$arOneProp['NAME'].'</b>'.(
			...
			<div class="b-products-table__quantity">
				<?=$arItem["QUANTITY"]?>
			</div>

			<div class="b-products-table__order">


				<? if (!isset($arItem['OFFERS']) || empty($arItem['OFFERS'])): ?>

					<? if ($arParams['USE_PRODUCT_QUANTITY'] == 'Y') :?>
						<div class="b-products-table__controls">
				...
				<? else: ?>
					...
					<? if ($arParams['PRODUCT_DISPLAY_MODE'] == "Y"): ?>
						...
						<div class="b-products-table__controls no_touch">
							<? //if ($canBuy): ?>
								<? if ($arParams['USE_PRODUCT_QUANTITY'] == 'Y') :?>
									<span>Количество:</span>
									<div class="b-catalog-list_item_controls_quantity">
				...
				<div class="b-products-table__price">
					<div id="<?=$arItemIDs['PRICE']; ?>" class="bx_price">
						<? if(!empty($minPrice)): ?>
							<? if('N' == $arParams['PRODUCT_DISPLAY_MODE'] && isset($arItem['OFFERS']) && !empty($arItem['OFFERS'])): ?>
								...
								<? $frame->end(); ?>
							<? else: ?>
								<div class="b-current-price g-accent-font">
									<? $frame = $this->createFrame()->begin(""); ?>
							...
							<? if('Y' == $arParams['SHOW_OLD_PRICE'] && $minPrice['DISCOUNT_VALUE'] < $minPrice['VALUE']): ?>
								<div class="b-old-price">
									<? $frame = $this->createFrame()->begin(""); ?>
										Старая цена: <?=$minPrice['PRINT_VALUE'];?>
									<? $frame->end(); ?>
								</div>
							<? endif; ?>
							<? unset($minPrice); ?> 
				...
				<? if (!isset($arItem['OFFERS']) || empty($arItem['OFFERS'])): ?>
					...
					<div class="b-products-table__actions">

						<? if($arParams['DISPLAY_COMPARE']): ?>
							<a class="b-products-table__button b-products-table__compare g-ajax-data" href="<?=$arItem["ADD_TO_COMPARE_URL"]?>"><?=GetMessage('CT_BCS_TPL_MESS_BTN_COMPARE'); ?></a>
						<? endif; ?>

						<? if($arParams["USE_FAVORITE"] == "Y"): ?>
							<div class="b-products-table__button b-products-table__favorite g-ajax-data">
						...
						<? if($arItem['CAN_BUY']): ?>
							...
							*/ ?>
							<?/*<a class="b-products-table__button b-products-table__basket g-button g-button--small g-ajax-data" href="<?=$arItem["ADD_TO_BASKET_URL"]?>" id="<?=$arItemIDs['BUY_LINK']; ?>" rel="nofollow">
								<? echo $arParams['ADD_TO_BASKET_ACTION'] == 'BUY' ? GetMessage('CT_BCS_TPL_MESS_BTN_BUY') : GetMessage('CT_BCS_TPL_MESS_BTN_ADD_TO_BASKET');?>
							</a>*/?>
							<div class="b-summ-price g-accent-font">
								<? $frame = $this->createFrame()->begin(""); ?>
									<?=$arItem["MIN_PRICE"]["PRINT_PRICE_FOR_QUANTITY"];?>
								<? $frame->end(); ?>	
							</div>
						<? else: ?>
							...
							<? if($showSubscribeBtn): ?>
								<div class="b-products-table__subscribe">
				...
				<? else: ?>
					...
					<? if ($arParams['PRODUCT_DISPLAY_MODE'] == "Y"): ?>
						...
						<div class="b-products-table__actions">
							...
							<a class="b-products-table__button b-products-table__basket g-button g-ajax-data" href="<?=$arItem['OFFERS']['ADD_TO_BASKET_URL']?>" id="<?=$arItemIDs['BUY_LINK']; ?>" rel="nofollow">
								<? echo $arParams['ADD_TO_BASKET_ACTION'] == 'BUY' ? GetMessage('CT_BCS_TPL_MESS_BTN_BUY') : GetMessage('CT_BCS_TPL_MESS_BTN_ADD_TO_BASKET');?>
							</a>
					...
					<div class="b-products-table__controls touch">
						<a href="<? echo $arItem['DETAIL_PAGE_URL']; ?>">
							<? echo GetMessage('CT_BCS_TPL_MESS_BTN_DETAIL');?>
						</a>
					</div>
...
<?/*
<div class="b-solution-order text-center">
	<span class="g-price"><?= 'от '.number_format($arItem['PROPERTIES']['PRICE']['VALUE'], 0, ',', ' ').' ₽'?></span>
	<a href="/catalog/add-to-basket/?action=add&id=<?= implode(',', $arItem['PROPERTIES']['SOLUTION_ITEMS']['VALUE']); ?>&quantity=<?= implode(',', $arItem['PROPERTIES']['SOLUTION_ITEMS']['DESCRIPTION']);?>" class="g-button g-ajax-data">Заказать проект</a>
	<? if ($arItem['DETAIL_TEXT']): ?>
		<button class="g-estimate" type="button"><span>Посмотреть смету</span></button>
	<? endif; ?>
</div>
*/?>
