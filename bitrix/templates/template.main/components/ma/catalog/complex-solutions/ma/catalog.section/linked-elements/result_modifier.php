...
if (!empty($arResult['ITEMS']))
{
	...
	foreach ($arResult['ITEMS'] as $key => $arItem) 
	{
		...
		// Количество связаных элементов
		$arItem["QUANTITY"] = $arParams["LINKED_QUANTITY_LIST"][$arItem['ID']];
		$arItem["MIN_PRICE"]["PRICE_FOR_QUANTITY"] = $arItem["MIN_PRICE"]["DISCOUNT_VALUE"] * $arItem["QUANTITY"];
		$arItem["MIN_PRICE"]["PRINT_PRICE_FOR_QUANTITY"] = CurrencyFormat($arItem["MIN_PRICE"]["DISCOUNT_VALUE"] * $arItem["QUANTITY"], "RUB");

		// Список ID и кол-во связаных эл-тов для добавляения в корзину
		$arResult['SOLUTION_ITEMS']['ID'][] = $arItem['ID'];
		$arResult['SOLUTION_ITEMS']['QUANTITY'][] = $arParams["LINKED_QUANTITY_LIST"][$arItem['ID']];
