...
$productSlider = CIBlockPriceTools::getSliderForItem($arResult, $arParams['ADD_PICT_PROP'], 'Y');
...
// ResizeImage 
else if (is_array($productSlider)) 
{
	foreach ($productSlider as $key => $productSliderPic)
	{

		$arPicture = CFile::GetFileArray($productSliderPic['ID']); 

		if( is_array($arPicture)) {

			if($arParams['DISPLAY_IMG_WIDTH'] <= 0 ){
 				$arParams['DISPLAY_IMG_WIDTH'] = 550;
 			}
 			if($arParams['DISPLAY_IMG_HEIGHT'] <= 0){
 				$arParams['DISPLAY_IMG_HEIGHT'] = 550;
 			}
 			...
 			if($arParams['USE_WATERMARK'] == 'Y' && !empty($arParams['WATERMARK_URL']))
 			{
				$arFilter = array(
					array(
						...
						"fill" => 'repeat', // exact / repeat (не должно быть "size" => "real") 
					)
				);
			}
		...
		$arResizePicture = array(
			'ID' => $productSliderPic['ID'],
			'SRC' => $arFileTmp['src'],
			'WIDTH' => $arFileTmp['width'],
			'HEIGHT' => $arFileTmp['height'],
		);
		$arPicture["PREVIEW_PICTURE"] = $arResizePicture;

		$productSlider[$key] = $arPicture;
	}
}
// ResizeImage 
...
if ($arResult['CATALOG'] && isset($arResult['OFFERS']) && !empty($arResult['OFFERS']))
{
	...
	foreach ($arResult['OFFERS'] as $keyOffer => $arOffer)
	{
		...
		foreach ($arOffer["MORE_PHOTO"] as $f => $arPicture) {
			$picture = CFile::GetFileArray($arPicture["ID"]);
			if( is_array($picture)) {

				if($arParams['DISPLAY_DOP_IMG_WIDTH'] <= 0 ){
	 				$arParams['DISPLAY_DOP_IMG_WIDTH'] = 150;
	 			}
	 			if($arParams['DISPLAY_DOP_IMG_HEIGHT'] <= 0){
	 				$arParams['DISPLAY_DOP_IMG_HEIGHT'] = 110;
	 			}
				...
				// }
	 			...
	 			$arFileTmp = CFile::ResizeImageGet(
	 				$picture,
	 				array(
	 					'width' => $arParams['DISPLAY_DOP_IMG_WIDTH'],
	 					'height' => $arParams['DISPLAY_DOP_IMG_HEIGHT']
					),
					$arParams['RESIZE_IMAGE'],
					true,
					$arFilter
			...
			$arOffer["MORE_PHOTO_RESIZE"][$f] = $arResizePicture;
		}
...
// 	}
// }
foreach ($arSKUPropList as &$arProp) {
	foreach ($arProp["VALUES"] as $key => $arValue) {
		if($arValue["ID"] <= 0) {
			unset($arProp["VALUES"][$key]);
		}
	}
...
$arElectricProps = array();
$arMechanicalProps = array();
$arEnvironmentConditionsProps = array();
$arProtectionProps = array();


$electricPropslist = $arParams["DETAIL_PROPERTY_CODE_2"];
$mechanicalPropslist = $arParams["DETAIL_PROPERTY_CODE_3"];
$environmentConditionsPropslist = $arParams["DETAIL_PROPERTY_CODE_4"];
$protectionPropslist = $arParams["DETAIL_PROPERTY_CODE_5"];

if (!empty($arResult['DISPLAY_PROPERTIES'])) {
	foreach ($arResult['DISPLAY_PROPERTIES'] as $arrProp) {
		if (in_array($arrProp["CODE"], $electricPropslist)) {
			$arElectricProps[] = $arrProp;
		}
		if (in_array($arrProp["CODE"], $mechanicalPropslist)) {
			$arMechanicalProps[] = $arrProp;
		}
		if (in_array($arrProp["CODE"], $environmentConditionsPropslist)) {
			$arEnvironmentConditionsProps[] = $arrProp;
		}
		if (in_array($arrProp["CODE"], $protectionPropslist)) {
			$arProtectionProps[] = $arrProp;
		}
	}
}

$arResult['DISPLAY_PROPERTIES']['ELECTRIC_PROPS'] = $arElectricProps;
$arResult['DISPLAY_PROPERTIES']['MECHANICAL_PROPS'] = $arMechanicalProps;
$arResult['DISPLAY_PROPERTIES']['ENVIRONMENT_CONDITIONS_PROPS'] = $arEnvironmentConditionsProps;
$arResult['DISPLAY_PROPERTIES']['PROTECTION_PROPS'] = $arProtectionProps;

$navChain = CIBlockSection::GetNavChain($arResult["IBLOCK_ID"], $arResult["IBLOCK_SECTION_ID"]); 
while ($arSection=$navChain->GetNext()) {
	$SECTIONS[] = $arSection;
}

$arResult["PARENT_SECTION"] = $SECTIONS[0]["ID"];