(function(window){
	...
	window.JCCatalogElement = function(arParams)
	{
		...
		this.sliderRowShowSize = 4; /* Количество элементов в карусели */
	...
	window.JCCatalogElement.prototype = {
		...
		{
			var j = 0, i, slider, images, img, id;
			...
			switch (this.config.mainPictureMode)
			{
				...
				case 'POPUP':
					try {
						...
						// Фенсибокс 2
						/*
						BX.bind(this.obPict, 'click', function() {
							$.fancybox({
								href: this.src,
								title : this.title
							});
		...
		fancyboxPopup: function()
		{
			...
			// } else if(this.productType === 1 || this.productType === 0) {
		...
		setCurrentImg: function(img, showImage)
		{
			...
			if (img.PREVIEW_PICTURE) {
				this.currentImg.src = img.PREVIEW_PICTURE.SRC;	
			} else {
				this.currentImg.src = '/bitrix/templates/template.main/components/ma/catalog/catalog/ma/catalog.element/offers/images/no_photo.png';
			}
...
$(document).ready(function() {
	var customBtn = $(".b-catalog-detail_detail-pic_arrow"),
		customBtnPrev = $(".b-catalog-detail_detail-pic_arrow.arrow-prev"),
		customBtnNext = $(".b-catalog-detail_detail-pic_arrow.arrow-next"),
		sectMorePhoto = $(".b-catalog-detail_more-pic"),
		morePhotoFirstItem = sectMorePhoto.find("li:first-child"),
		morePhotoLastItem = sectMorePhoto.find("li:last-child");

	sectMorePhoto.find('li').on("click", function() {
		if (!$(this).is(":first-child") || !$(this).is(":last-child")) {
			customBtn.removeClass('disabled');
		}
	})
	morePhotoFirstItem.on("click", function(event) {
		customBtnPrev.addClass('disabled');
	});

	morePhotoLastItem.on("click", function(event) {
		customBtnNext.addClass('disabled');
	});

	$(".b-catalog-detail_detail-pic_arrow").on("click", function(event) {
		if ($(this).hasClass('arrow-prev')) {
			customBtnNext.removeClass('disabled');
			sectMorePhoto.find(".active").prev().trigger('click');
		}
		if ($(this).hasClass('arrow-next')) {
			customBtnPrev.removeClass('disabled');
			sectMorePhoto.find(".active").next().trigger('click');
		}
	});

	var btn = $(".item_buttons_counter_block .g-button");
