...
// Фильтр по свойствам
if(is_array($arParams['FILTER_PROPERTIES']) && !empty($arParams['FILTER_PROPERTIES'])){
    foreach($arParams['FILTER_PROPERTIES'] as $filterItem){
        $arrFilter['!PROPERTY_'.$filterItem] = false;
    }
...
if($this->StartResultCache(false, array($arrFilter, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $arNavigation, $pagerParameters)))
{
	...
	if($arParams["BY_LINK"]!=="Y")
	{
		...
		elseif(empty($arrFilter)) // если не применяется фильтр
		{
	...
	if($arParams['USE_URL_IN_SETTINGS'] != 'Y'){
