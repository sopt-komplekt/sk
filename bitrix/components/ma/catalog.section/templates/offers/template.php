...
<? $templateLibrary = array('popup');
	...
	if (!empty($arResult['SKU_PROPS']))
	{
		foreach ($arResult['SKU_PROPS'] as $arProp)
		{
			...
			{
				$skuTemplate[$propId]['SCROLL']['START'] = '<div class="b-catalog-sections_item_scu_size full" id="#ITEM#_prop_'.$propId.'_cont">'.
					'<span class="b-catalog-sections_item_scu_prop_title">'.htmlspecialcharsbx($arProp['NAME']).'</span>'.
				...
				$skuTemplate[$propId]['FULL']['START'] = '<div class="b-catalog-sections_item_scu_size" id="#ITEM#_prop_'.$propId.'_cont">'.
					'<span class="b-catalog-sections_item_scu_prop_title">'.htmlspecialcharsbx($arProp['NAME']).'</span>'.
					'<div class="bx_size_scroller_container"><div class="bx_size"><ul id="#ITEM#_prop_'.$propId.'_list" style="width: #WIDTH#;">';;
			...
			{
				$skuTemplate[$propId]['SCROLL']['START'] = '<div class="b-catalog-sections_item_scu_pic full" id="#ITEM#_prop_'.$propId.'_cont">'.
					'<span class="b-catalog-sections_item_scu_prop_title">'.htmlspecialcharsbx($arProp['NAME']).'</span>'.
				...
				$skuTemplate[$propId]['FULL']['START'] = '<div class="b-catalog-sections_item_scu_pic" id="#ITEM#_prop_'.$propId.'_cont">'.
					'<span class="b-catalog-sections_item_scu_prop_title">'.htmlspecialcharsbx($arProp['NAME']).'</span>'.
					'<div class="bx_scu_scroller_container"><div class="bx_scu"><ul id="#ITEM#_prop_'.$propId.'_list" style="width: #WIDTH#;">';
				$skuTemplate[$propId]['FULL']['FINISH'] = '</ul></div>'.
					'<div class="bx_slide_left" id="#ITEM#_prop_'.$propId.'_left" data-treevalue="'.$propId.'" style="display: none;"></div>'.
...
<div class="b-catalog-sections">
	...
	?>
		...
		<div class="b-catalog-sections_item  b-csi-<?=$arItem['ID']?>">
			<div class="b-catalog-sections_item_holder" id="<?=$strMainID;?>">
				...
				<div class="b-catalog-sections_item_pic">
					<a 
						...
						title="<?=$imgTitle;?>"
						<? if($arItem['PREVIEW_PICTURE']['SRC']): ?>style="background-image: url('<?=$arItem['PREVIEW_PICTURE']['SRC']?>')"<? endif; ?>
					>
						<? if ($arParams['SHOW_DISCOUNT_PERCENT'] == 'Y'): ?>
							<span id="<?=$arItemIDs['DSC_PERC'];?>" class="bx_stick_disc right bottom" style="display:<? echo (0 < $minPrice['DISCOUNT_DIFF_PERCENT'] ? '' : 'none'); ?>;">-<?=$minPrice['DISCOUNT_DIFF_PERCENT'];?>%</span>
						<? endif; ?>
						<? if ($arItem['LABEL']): ?>
							<span id="<?=$arItemIDs['STICKER_ID'];?>" class="b-catalog-sections_item_stick" title="<?=$arItem['LABEL_VALUE']; ?>"><?=$arItem['LABEL_VALUE']; ?></span>
						<? endif; ?>
					</a>
				</div>
					...
					<div id="<?=$arItemIDs['PRICE']; ?>" class="bx_price">
						<? if(!empty($minPrice)): ?>
							...
							<? else: ?>
								...
								<div class="b-current-price"><?=$minPrice['PRINT_DISCOUNT_VALUE'];?></div>
							<? endif; ?>
							<? if('Y' == $arParams['SHOW_OLD_PRICE'] && $minPrice['DISCOUNT_VALUE'] < $minPrice['VALUE']): ?>
								<?/*<span><?=$minPrice['PRINT_VALUE'];?></span>*/?>
								<div class="b-old-price"><?=$minPrice['PRINT_VALUE'];?></div>
							<? endif; ?>
							<? unset($minPrice); ?> 
						<? endif; ?>
					</div>
				...
				?>
						...
						<? if($arItem['CAN_BUY']): ?>
							<? if('Y' == $arParams['USE_PRODUCT_QUANTITY']): ?>
								<div class="b-catalog-sections_item_controls_quantity">
									<a id="<?=$arItemIDs['QUANTITY_DOWN']; ?>" href="javascript:void(0)" class="b-catalog-sections_item_controls_quantity_minus" rel="nofollow">–</a>
									<input type="text" class="b-catalog-sections_item_controls_quantity_input" id="<?=$arItemIDs['QUANTITY']; ?>" name="<?=$arParams["PRODUCT_QUANTITY_VARIABLE"]; ?>" value="<?=$arItem['CATALOG_MEASURE_RATIO']; ?>">
									<a id="<?=$arItemIDs['QUANTITY_UP']; ?>" href="javascript:void(0)" class="b-catalog-sections_item_controls_quantity_plus" rel="nofollow">+</a>
									<span id="<?=$arItemIDs['QUANTITY_MEASURE']; ?>"></span>
								</div>
							<? endif; ?>
							<div id="<?=$arItemIDs['BASKET_ACTIONS']; ?>" class="b-catalog-sections_item_controls_basket">
								<a id="<?=$arItemIDs['BUY_LINK']; ?>" class="g-button g-ajax-data" href="<?=$arItem["ADD_TO_BASKET_URL"]?>" rel="nofollow">
								<?
							...
							<? if ($arParams["USE_COMPARE"] == "Y"): ?>
								<div class="b-catalog-sections_item_controls_compare">
									<a class="g-ajax-data" href="<?=$arItem["ADD_TO_COMPARE_URL"]?>"><?=$compareBtnMessage; ?></a>
								</div>
							<? endif; ?>
							<? if ($arParams["USE_FAVORITE"] == "Y"): ?>
								<div class="b-catalog-sections_item_controls_favorite">
									<a class="g-ajax-data" href="<?=$arItem["ADD_TO_FAVORITE_URL"]?>"><? echo GetMessage('MA_CSO_FAVORITE_BUTTON'); ?></a>
								</div>
							<? endif; ?>
						<? else: ?>
							<div id="<?=$arItemIDs['NOT_AVAILABLE_MESS'];?>" class="b-catalog-sections_item_controls_notavailable">
								<span class="bx_notavailable">
									<? echo ('' != $arParams['MESS_NOT_AVAILABLE'] ? $arParams['MESS_NOT_AVAILABLE'] : GetMessage('CT_BCS_TPL_MESS_PRODUCT_NOT_AVAILABLE'));?>
								</span>
							</div>
							<? if($arParams["USE_COMPARE"] == "Y" || $showSubscribeBtn): ?>
								<div class="b-catalog-sections_item_controls_compare">
									<? if($arParams["USE_COMPARE"] == "Y"): ?>
					...
					<? if (isset($arItem['DISPLAY_PROPERTIES']) && !empty($arItem['DISPLAY_PROPERTIES'])): ?>
						<div class="b-catalog-sections_item_articul">
			...
			<? else: ?>
				...
				?>
							...
							<div class="b-catalog-sections_item_scu" id="<? echo $arItemIDs['PROP_DIV']; ?>">
								<?
								foreach ($skuTemplate as $propId => $propTemplate)
								{
									...
									unset($valueCount);
									echo '<div class="b-catalog-sections_item_scu_prop">', str_replace(array('#ITEM#_prop_', '#WIDTH#'), array($arItemIDs['PROP'], $fullWidth), $rowTemplate['START']);
				...
				?>
					<div class="b-catalog-sections_item_controls no_touch">
						<? if ('Y' == $arParams['USE_PRODUCT_QUANTITY']) :?>
							<div class="b-catalog-sections_item_controls_quantity">
								<a id="<?=$arItemIDs['QUANTITY_DOWN']; ?>" href="javascript:void(0)" class="b-catalog-sections_item_controls_quantity_minus" rel="nofollow">–</a>
								<input type="text" class="b-catalog-sections_item_controls_quantity_input" id="<?=$arItemIDs['QUANTITY']; ?>" name="<?=$arParams["PRODUCT_QUANTITY_VARIABLE"]; ?>" value="<?=$arItem['CATALOG_MEASURE_RATIO']; ?>">
						...
						<div id="<?=$arItemIDs['NOT_AVAILABLE_MESS']; ?>" class="b-catalog-sections_item_controls_quantity" style="display: <? echo ($canBuy ? 'none' : ''); ?>;">
							<span class="bx_notavailable">
								<? echo ('' != $arParams['MESS_NOT_AVAILABLE'] ? $arParams['MESS_NOT_AVAILABLE'] : GetMessage('CT_BCS_TPL_MESS_PRODUCT_NOT_AVAILABLE')); ?>
							</span>
						</div>

						<div id="<?=$arItemIDs['BASKET_ACTIONS']; ?>" class="b-catalog-sections_item_controls_basket" style="display: <? echo ($canBuy ? '' : 'none'); ?>;">
							<a id="<?=$arItemIDs['BUY_LINK']; ?>" class="g-button g-ajax-data" href="<?=$arItem['OFFERS']["ADD_URL"]?>" rel="nofollow">
							<?
								if ($arParams['ADD_TO_BASKET_ACTION'] == 'BUY') {
									echo ('' != $arParams['MESS_BTN_BUY'] ? $arParams['MESS_BTN_BUY'] : GetMessage('CT_BCS_TPL_MESS_BTN_BUY'));
								}
						...
						<? if ($arParams["USE_COMPARE"] == "Y"): ?>
							<div class="b-catalog-sections_item_controls_compare">
								<a id="<?=$arItemIDs['COMPARE_LINK']; ?>" href="javascript:void(0)"><?=$compareBtnMessage; ?></a>
							</div>
						<? endif; ?>
				...
				<div class="b-catalog-sections_item_controls touch">
					<a href="<? echo $arItem['DETAIL_PAGE_URL']; ?>">
						<? echo ('' != $arParams['MESS_BTN_DETAIL'] ? $arParams['MESS_BTN_DETAIL'] : GetMessage('CT_BCS_TPL_MESS_BTN_DETAIL'));?>
					</a>
				</div>
