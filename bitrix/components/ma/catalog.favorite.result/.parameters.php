...
$arPaths = array(
	$_SERVER['DOCUMENT_ROOT'].SITE_TEMPLATE_PATH.'/components/ma/catalog.section/templates/',
	$_SERVER['DOCUMENT_ROOT'].'/bitrix/templates/.default/components/ma/catalog.section/templates/',
	$_SERVER['DOCUMENT_ROOT'].'/bitrix/components/ma/catalog.section/templates/',
);
foreach ($arPaths as $key => $path) {
	if(file_exists($path)) {
...
$rsProp = CIBlockProperty::GetList(Array("sort"=>"asc", "name"=>"asc"), Array("ACTIVE"=>"Y", "IBLOCK_ID"=>$arCurrentValues["IBLOCK_ID"], "PROPERTY_TYPE"=>"L", "LIST_TYPE"=>"C"));
while ($arr=$rsProp->Fetch()){
	$arPropertyFilter[$arr["CODE"]] = "[".$arr["CODE"]."] ".$arr["NAME"];
}
...
$arComponentParameters = array(
	...
	"PARAMETERS" => array(
		"ELEMENTS_LIST_TEMPLATE" => array(
			"PARENT" => "COMPONENT_TEMPLATE",
			"NAME" => GetMessage("CP_BCS_ELEMENTS_LIST_TEMPLATE"),
			"TYPE" => "LIST",
...
if($arPropertyFilter)
{    
    $arComponentParameters["GROUPS"]["FILTER_PROPERTIES"] = array(
        "NAME" => GetMessage("FILTER_PROPERTIES")
    );
	$arComponentParameters["PARAMETERS"]['FILTER_PROPERTIES'] = array(
        "PARENT" => "FILTER_PROPERTIES",
		"NAME" => GetMessage("FILTER_PROPERTIES_LIST"),
		"TYPE" => "LIST",
		"MULTIPLE" => "Y",
		"VALUES" => $arPropertyFilter,
    );
