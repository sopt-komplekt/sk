	...
	if(is_array($arResult["ITEMS"]) && !empty($ELEMENT_ID)){
		foreach ($arResult["ITEMS"] as $key => $arItem) {
			if ($arItem['ID'] == $ELEMENT_ID) {
				...
				if ('add' != $ACTION) $removedKey = $key;
				break;
			}
		}
		if ($removedKey) unset($arResult['ITEMS'][$removedKey]);
	}
...
<div class="b-added-item">
	<div class="b-added-item_title h2">
		<?= ('add' != $ACTION) ? GetMessage('MA_CATALOG_FAVORITE_RESULT_REMOVE') : GetMessage('MA_CATALOG_FAVORITE_RESULT_ADD'); ?>
	</div>
