...
<div class="bx_sort_container">
	...
	<a class="<? echo (!$arResult["DIFFERENT"] ? 'current' : ''); ?>" href="<? echo $arResult['COMPARE_URL_TEMPLATE'].'DIFFERENT=N'; ?>" rel="nofollow">
		<span><?=GetMessage("CATALOG_ALL_CHARACTERISTICS")?></span>
	</a>
	<a class="<? echo ( $arResult["DIFFERENT"] ? 'current' : ''); ?>" href="<? echo $arResult['COMPARE_URL_TEMPLATE'].'DIFFERENT=Y'; ?>" rel="nofollow">
					...
					case "DETAIL_PICTURE":
						if(is_array($arElement["FIELDS"][$code])):?>
							<a href="<?=$arElement["DETAIL_PAGE_URL"]?>">
								<img src="<?=$arElement["FIELDS"][$code]["SRC"]?>" width="<?=$arElement["FIELDS"][$code]["WIDTH"]?>" height="<?=$arElement["FIELDS"][$code]["HEIGHT"]?>" alt="<?=$arElement["NAME"]?>">
							</a>
						<?endif;
						break;
					default:
		...
		<td>
			<div style="margin-bottom: 10px; text-align: center;">
				<?if($arElement["CAN_BUY"]):?>
					<a class="g-button" href="<?=$arElement["BUY_URL"]?>" rel="nofollow"><?=GetMessage("CATALOG_COMPARE_BUY"); ?></a>
				<?elseif(!empty($arResult["PRICES"]) || is_array($arElement["PRICE_MATRIX"])):?>
					<div style="line-height: 40px;"><?=GetMessage("CATALOG_NOT_AVAILABLE")?></div>
				<?endif;?>
			</div>
			<div style="text-align: center;">
				<a href="javascript:void(0)" onclick="CatalogCompareObj.delete('<?=CUtil::JSEscape($arElement['~DELETE_URL'])?>');" ><?=GetMessage("CATALOG_REMOVE_PRODUCT")?></a>
			</div>
		</td>
		<?
		}
