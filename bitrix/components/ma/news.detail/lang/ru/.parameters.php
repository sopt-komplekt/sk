<?
$MESS["T_IBLOCK_DESC_LIST_ID"] = "Код информационного блока";
$MESS["T_IBLOCK_DESC_LIST_TYPE"] = "Тип информационного блока (используется только для проверки)";
$MESS["T_IBLOCK_DESC_LIST_PAGE_URL"] = "URL страницы просмотра списка элементов (по умолчанию - из настроек инфоблока)";
$MESS["CP_BND_DETAIL_URL"] = "URL страницы детального просмотра (по умолчанию - из настроек инфоблока)";
$MESS["T_IBLOCK_DESC_INCLUDE_IBLOCK_INTO_CHAIN"] = "Включать инфоблок в цепочку навигации";
$MESS["T_IBLOCK_PROPERTY"] = "Свойства";
$MESS["T_IBLOCK_DESC_KEYWORDS"] = "Установить ключевые слова страницы из свойства";
$MESS["T_IBLOCK_DESC_DESCRIPTION"] = "Установить описание страницы из свойства";
$MESS["IBLOCK_FIELD"] = "Поля";
$MESS["T_IBLOCK_DESC_ACTIVE_DATE_FORMAT"] = "Формат показа даты";
$MESS["T_IBLOCK_DESC_USE_PERMISSIONS"] = "Использовать дополнительное ограничение доступа";
$MESS["T_IBLOCK_DESC_GROUP_PERMISSIONS"] = "Группы пользователей, имеющие доступ к детальной информации";
$MESS["T_IBLOCK_DESC_PAGER_PAGE"] = "Страница";
$MESS["T_IBLOCK_DESC_ADD_SECTIONS_CHAIN"] = "Включать раздел в цепочку навигации";
$MESS["T_IBLOCK_DESC_ADD_ELEMENT_CHAIN"] = "Включать название элемента в цепочку навигации";
$MESS["T_IBLOCK_DESC_CHECK_DATES"] = "Показывать только активные на данный момент элементы";
$MESS["CP_BND_ELEMENT_CODE"] = "Код новости";
$MESS["CP_BND_ELEMENT_ID"] = "ID новости";
$MESS["CP_BND_BROWSER_TITLE"] = "Установить заголовок окна браузера из свойства";
$MESS["CP_BND_CACHE_GROUPS"] = "Учитывать права доступа";
$MESS["CP_BND_SET_BROWSER_TITLE"] = "Устанавливать заголовок окна браузера";
$MESS["CP_BND_SET_META_KEYWORDS"] = "Устанавливать ключевые слова страницы";
$MESS["CP_BND_SET_META_DESCRIPTION"] = "Устанавливать описание страницы";
$MESS["CP_BND_SET_CANONICAL_URL"] = "Устанавливать канонический URL";
$MESS["CP_BND_SET_LAST_MODIFIED"] = "Устанавливать в заголовках ответа время модификации страницы";
$MESS["CP_BND_STRICT_SECTION_CHECK"] = "Строгая проверка раздела для показа элемента";

$MESS['DISPLAY_IMG_WIDTH'] = 'Ширина изображения';
$MESS['DISPLAY_IMG_HEIGHT'] = 'Высота изображения';
$MESS["RESIZE_IMAGE"] = "Тип масштабирования изображений";
$MESS["BX_RESIZE_IMAGE_EXACT"] = "В прямоугольник без сохранения пропорций";
$MESS["BX_RESIZE_IMAGE_PROPORTIONAL"] = "С сохранением пропорций";
$MESS["BX_RESIZE_IMAGE_PROPORTIONAL_ALT"] = "С сохранением пропорций, улучшенная обработка верт. картинок";

$MESS["DISPLAY_DOP_IMG_WIDTH"] = "Ширина картинки для дополнительных изображений";
$MESS["DISPLAY_DOP_IMG_HEIGHT"] = "Высота картинки для дополнительных изображений";
$MESS["RESIZE_IMAGE_DOP"] = "Тип масштабирования дополнительных изображений";
$MESS["ADD_PICT_PROP"] = "Свойство, где хранятся дополнительный изображения";
$MESS["CP_BC_TPL_PROP_EMPTY"] = "Не выбрано";

?>