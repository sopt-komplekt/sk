...
$COMPLEX_COMPONENT_PATH = '/bitrix/components/ma/catalog/';
$APPLICATION->AddHeadScript($COMPLEX_COMPONENT_PATH.'/script.js');
...
if (count($arResult['OFFERS_IDS']) > 0 && CModule::IncludeModule('sale'))
{
	...
	if (count($arPageItems) > 0 || count($arPageItemsDelay) > 0)
	{
		...
		foreach ($arPageItems as $id) 
		{
			echo "disableAddToCart('catalog_add2cart_link_ofrs_".$id."', '".GetMessage("CATALOG_IN_CART")."');\r\n";
		}
		foreach ($arPageItemsDelay as $id) 
		{
