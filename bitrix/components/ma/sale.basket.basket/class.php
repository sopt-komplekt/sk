...
class CBitrixBasketComponent extends CBitrixComponent
{
	...
	public function onPrepareComponentParams($arParams)
	{
		...
		// Properties columns after "NAME" column
		if (!empty($arParams['COLUMNS_LIST']) && is_array($arParams['COLUMNS_LIST'])) {
			foreach ($arParams['COLUMNS_LIST'] as $key => $COLUMN_NAME) {
				if (strpos($COLUMN_NAME, "PROPERTY_") !== false) {
					unset($arParams['COLUMNS_LIST'][$key]);
					array_splice($arParams['COLUMNS_LIST'], 1, 0, $COLUMN_NAME);
				}
