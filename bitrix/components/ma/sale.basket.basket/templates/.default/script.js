		...
		for (i = 0; i < arColumns.length; i++)
		{
			switch (arColumns[i])
			{
				...
				case 'NAME':
					...
					{
						cellNameHTML = '<div class="b-basket_table_photo-container">\
					...
					{
						cellNameHTML += '<div class="b-basket_table_brand">\
					...
					if (arItem['DETAIL_PAGE_URL'].length > 0)
						cellItemHTML += '<div class="b-basket_table_item-name"><a href="' + arItem['DETAIL_PAGE_URL'] + '">' + arItem['NAME'] + '</a></div>';
					else
						cellItemHTML += '<div class="b-basket_table_item-name">' + arItem['NAME'] + '</div>';
		...
		'div',
		{
			...
			children: [
				...
				BX.create(
					...
					{
						props: {
							className: 'b-basket_coupon_notes'
						},
...
function skuPropClickHandler(e)
{
	...
	if (!!target && target.hasAttribute('data-value-id'))
	{
		...
		BX.ajax({
			url: '/bitrix/components/ma/sale.basket.basket/ajax.php',
			method: 'POST',
			data: postData,
			dataType: 'json',
