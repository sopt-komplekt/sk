...
class MARegionsIPGeoBase 
{
	...
	function __construct($CIDRFile = false, $CitiesFile = false)
	{
		if(!$CIDRFile)
		{
			$CIDRFile = dirname(__FILE__) . '/data/cidr_optim.txt';			
		}
		if(!$CitiesFile)
		{
			$CitiesFile = dirname(__FILE__) . '/data/cities.txt';			
		}
	...
	function getCityByIdx($idx)
	{
		...
		while(!feof($this->fhandleCities))
		{
			...
			if($arRecord[0] == $idx)
			{
				foreach ($arRecord as $key => $value) {
					$get = '';
					$get = mb_detect_encoding($value, array('utf-8', 'cp1251'));
					$arRecord[$key] = iconv($get,'UTF-8',$value);
				}
	...
	*/
	function getCityList()
	{
	    ...
	    while (!feof($this->fhandleCities))
	    {
	        ...
	        $arRecord   = explode("\t", trim($str));
	        $get = '';
	        $get = mb_detect_encoding($arRecord[1], array('utf-8', 'cp1251'));
	        $cityList[$arRecord[0]] = array(
	        	'ID' => $arRecord[0],
	        	'CITY' => iconv($get,'UTF-8',$arRecord[1]),
	        	'REGION' => iconv($get,'UTF-8',$arRecord[2]),
	        	'DISTRICT' => iconv($get,'UTF-8',$arRecord[3]),
	        );
	    }
	    return $cityList;
	}
