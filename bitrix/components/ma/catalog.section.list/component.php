...
// Костыль для очистки кэша и установки активной страницы
if($arParams['SEF_MODE'] == 'Y')
{
	...
	if(!empty($arVariables['SECTION_CODE_PATH'])){
		$arParams['VARIABLES']['SECTION_ID'] = $arVariables['SECTION_CODE_PATH'];
	}
	else {
		$arParams['VARIABLES']['SECTION_ID'] = $arVariables['SECTION_ID'];
	}
}
else {
	$arParams['VARIABLES']['SECTION_ID'] = $_REQUEST['SECTION_ID'];
}

//if($this->StartResultCache(false, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups())))
if($this->StartResultCache(false, ($arParams["CACHE_GROUPS"]==="N"? false: $arParams['VARIABLES']['SECTION_ID'])))
{
	...
	if(is_array($arResult["SECTION"]))
	{
		unset($arFilter["ID"]);
		unset($arFilter["=CODE"]);
		if ((($arResult['SECTION']['RIGHT_MARGIN'] - $arResult['SECTION']['LEFT_MARGIN']) == 1 ) && $arParams['HIDDEN_SECTION_LIST_WITHOUT_SUBSECTIONS'] != "Y") {
			// В разделе нет подразделов. Выводим соседние разделы вместе с текущим, если в настройках не указано скрыть
				$arFilter['SECTION_ID'] = $arResult['SECTION']['IBLOCK_SECTION_ID'];
