...
if(!is_array($arParams["REQUIRED_USER_FIELDS"]))
	$arParams["REQUIRED_USER_FIELDS"] = array();
...
$arResult["REQUIRED_FIELDS"] = array_merge($arDefaultFields, $arParams["REQUIRED_FIELDS"], $arParams['REQUIRED_USER_FIELDS']);
...
// register user
//if ($_SERVER["REQUEST_METHOD"] == "POST" && !empty($_REQUEST["register_submit_button"]) && !$USER->IsAuthorized()) // плохо работает при аяксе
if ($_SERVER["REQUEST_METHOD"] == "POST" && !empty($_REQUEST['submited']) && !$USER->IsAuthorized())
{
