...
<? if($arResult['FORM_ITEMS'] && $arResult['MESSAGE_RESULT'] != 'MESSAGE_RESULT_OK'): ?>
	...
	<div class="b-message-form b-message-form_<?=$arParams['IBLOCK_ID']?>">
		<form method="post" action="<?=$arParams['ACTION_FORM']?>#<?=$arResult['FORM_ID']?>" enctype="multipart/form-data">
			...
			<? foreach($arResult['FORM_ITEMS'] as $keyItem => $arProperties): ?>
				...
				<div class="b-message-form_item<? if($arProperties['ERROR']): ?> b-message-form_error<? endif ?>" id="i-message-form-item-<?=$arProperties['ID']?>">
					...
					<? elseif(($arProperties['PROPERTY_TYPE'] == 'S' || $arProperties['PROPERTY_TYPE'] == 'N') && $arProperties['MULTIPLE'] == 'Y'): ?>
						<div class="b-message-form_field b-message-form_text">
							<? for ($i=0; $i < $arProperties['MULTIPLE_CNT']; $i++): ?>
								<div class="b-message-form_field b-message-form_text_item">
									<input id="i-message-form-field-<?=$arProperties['ID']?>-<?=$i?>" class="inputtext" name="FIELD_<?=$arProperties['ID']?>[]" type="text" value="<?=$arResult['POST']['FIELD_'.$arProperties['ID']][$i]?>" />
								</div>
							<? endfor; ?>
						</div>
					<? elseif($arProperties['PROPERTY_TYPE'] == 'S' || $arProperties['PROPERTY_TYPE'] == 'N'): ?>
						<div class="b-message-form_field b-message-form_text">
							<input id="i-message-form-field-<?=$arProperties['ID']?>" class="inputtext" name="FIELD_<?=$arProperties['ID']?>" type="text" value="<?=$arResult['POST']['FIELD_'.$arProperties['ID']]?>" />
						</div>
					...
					<? elseif($arProperties['PROPERTY_TYPE'] == 'F' && $arProperties['MULTIPLE'] != 'Y'): ?>
						<div class="b-message-form_field b-message-form_file">
							<? 
								if($arResult['POST']['FIELD_'.$arProperties['ID']]['SRC']): 
								$arFileImg = $arResult['POST']['FIELD_'.$arProperties['ID']];
							?>
								<div class="b-message-form_file_pic">
									Загружен: <a class="g-fancybox g-half-link" href="<?=$arFileImg['SRC']?>"><?=$arFileImg['ORIGINAL_NAME']?></a> (<?=HumanBytes($arFileImg['FILE_SIZE'])?>)
								</div>
							<? endif; ?>
							<input id="i-message-form-field-<?=$arProperties['ID']?>" class="inputfile" name="FIELD_<?=$arProperties['ID']?>" type="file">
						</div>
					<? elseif($arProperties['PROPERTY_TYPE'] == 'F' && $arProperties['MULTIPLE'] == 'Y'): ?>
						<? for ($i=0; $i < $arProperties['MULTIPLE_CNT']; $i++): ?> 
							<div class="b-message-form_field b-message-form_file">
								<? 
									if($arResult['POST']['FIELD_'.$arProperties['ID']][$i]['SRC']): 
									$arFileImg = $arResult['POST']['FIELD_'.$arProperties['ID']][$i];
								?>
									<div class="b-message-form_file_pic">
										Загружен: <a class="g-fancybox g-half-link" href="<?=$arFileImg['SRC']?>"><?=$arFileImg['ORIGINAL_NAME']?></a> (<?=HumanBytes($arFileImg['FILE_SIZE'])?>)
									</div>
								<? endif; ?>
								<input id="i-message-form-field-<?=$arProperties['ID']?>" class="inputfile" name="FIELD_<?=$arProperties['ID']?>_<?=$i?>" type="file" value="" />
							</div>
						<? endfor ?>
			...
			<div class="b-message-form_submit">
				<button class="g-button" type="submit" value="<?=$arParams['TEXT_BUTTON']?>"><span><?=$arParams['TEXT_BUTTON']?></span></button>
			</div>
