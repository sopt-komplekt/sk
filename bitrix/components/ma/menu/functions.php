...
{
	function _GetChildMenuRecursive(&$arMenu, &$arResult, $menuType, $use_ext, $menuTemplate, $currentLevel, $maxLevel, $bMultiSelect, $displayAllsubMenu = 'N')
	{
		...
		for ($menuIndex = 0, $menuCount = count($arMenu); $menuIndex < $menuCount; $menuIndex++)
		{
			...
			//Menu from files
			$subMenuExists = false;
			//if($currentLevel < $maxLevel && $arMenu[$menuIndex]["SELECTED"])
			if ( ($displayAllsubMenu == 'Y' && $currentLevel < $maxLevel) || ($displayAllsubMenu == 'N' && $currentLevel < $maxLevel && $arMenu[$menuIndex]["SELECTED"]))
			{
				...
				if($bDir)
				{
					...
					if ($subMenuExists)
					{
						...
						if(count($menu->arMenu) > 0)
							_GetChildMenuRecursive($menu->arMenu, $arResult, $menuType, $use_ext, $menuTemplate, $currentLevel+1, $maxLevel, $bMultiSelect, $displayAllsubMenu);
					}
				}
			}
