...
(function (exports,main_popup,main_core) {
	  ...
	  babelHelpers.createClass(CreationMenu, [{
	    ...
	    value: function getPopup() {
	      ...
	      if (!this.popup) {
	        ...
	        this.popup = new main_popup.PopupWindow(this.getId(), options.bindElement, options);
	      }
	...
	function () {
	  ...
	  babelHelpers.createClass(Field, [{
	    ...
	    key: "isShowAlways",
	    value: function isShowAlways() {
	      return this.data.IS_SHOW_ALWAYS === 'Y';
	    }
	  }, {
	    key: "setIsShowAlways",
	    value: function setIsShowAlways(isShowAlways) {
	      this.data.IS_SHOW_ALWAYS = main_core.Text.toBoolean(isShowAlways) === true ? 'Y' : 'N';
	    }
	  }, {
	    key: "isMultiple",
	...
	function () {
	  ...
	  babelHelpers.createClass(Configurator, [{
	    ...
	    value: function renderOptions() {
	      ...
	      this.showAlwaysCheckbox = main_core.Tag.render(_templateObject16());
	      this.showAlwaysCheckbox.checked = this.field.isShowAlways();
	      this.optionsContainer.appendChild(main_core.Tag.render(_templateObject17(), this.showAlwaysCheckbox, main_core.Loc.getMessage('UI_USERFIELD_FACTORY_FIELD_SHOW_ALWAYS')));
	...
	function () {
	  function Factory(entityId) {
	    ...
	    if (main_core.Type.isPlainObject(params)) {
	      ...
	      if (main_core.Type.isObject(params.configuratorClass) && params.configuratorClass instanceof Configurator) {
	  ...
	  babelHelpers.createClass(Factory, [{
	    ...
	    value: function createField(fieldType, fieldName) {
	      ...
	      main_core.Event.EventEmitter.emit('UI.UserFieldFactory.Factory.onCreateField', {
	        factory: this,
	        field: field
	      });
	      return field;
	    ...
	    value: function saveField(field) {
	      ...
	      return new Promise(function (resolve, reject) {
	        if (field instanceof Field) {
	          if (field.isSaved()) {
	            BX.Main.UF.EditManager.update({
	              "FIELDS": [field.getData()]
	            }, function (response) {
	              _this.onFieldSave(response, resolve, reject);
	            });
	          } else {
	            BX.Main.UF.EditManager.add({
	              "FIELDS": [field.getData()]
	            }, function (response) {
	              _this.onFieldSave(response, resolve, reject);
	            });
	    ...
	    key: "onFieldSave",
	    value: function onFieldSave(response, onSuccess, onError) {
	      if (main_core.Type.isPlainObject(response)) {
	        if (response.ERROR && main_core.Type.isArray(response.ERROR) && response.ERROR.length > 0) {
	          ...
	          if (fieldData) {
	            ...
	            main_core.Event.EventEmitter.emit('UI.UserFieldFactory.Factory.onFieldSave', {
	              factory: this,
	              field: field
	            });
	          }
	    ...
	    value: function getFieldTypes() {
	      ...
	      var eventName = 'UI:UserFieldFactory:Factory:OnGetAdditionalUserTypes';
	      var additionalItems = [];
	      main_core.Event.EventEmitter.emit(eventName, {
	        additionalItems: additionalItems
	      });

	      if (main_core.Type.isArray(additionalItems)) {
	        additionalItems.forEach(function (_ref) {
	          var name = _ref.name,
	              title = _ref.title,
	              description = _ref.description;

	          if (main_core.Type.isString(name) && main_core.Type.isString(title) && main_core.Type.isString(description)) {
	            types.push({
	              name: name,
	              title: title,
	...
	exports.UserFieldFactory = UserFieldFactory;
